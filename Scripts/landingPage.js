...
export function renderLandingPage(username) {
  ...
  controls.innerHTML = `
    ...
    <div class="form-grid">
      ...
      <div class="field"><label>Main title</label>
        <input id="title" placeholder="Bannerist helps you launch faster"/>
      </div>

      <div class="field"><label>Subtitle</label>
        <input id="subtitle" placeholder="Create and manage ads, emails & landings in minutes."/>
      </div>

      <div class="field"><label>Paragraph</label>
        <textarea id="body" placeholder="Design quickly, preview instantly, and save your work locally."></textarea>
      </div>
      ...
      <div class="field"><label>CTA Text</label>
        <input id="ctaText" placeholder="Start free"/>
      </div>
      ...
      <h3 style="grid-column:1/-1;margin:16px 0 8px">Lead Form</h3>
      <div class="field">
        <label>Form Title</label>
        <input id="leadTitle" placeholder="Stay updated"/>
      </div>
      <div class="field">
  ...
  const btn = (text, url, accent) =>
    `<a href="${
      url || "#"
    }" style="display:inline-block;padding:12px 18px;border-radius:10px;background:${accent};color:#fff;text-decoration:none;font-weight:700">${
      text || "CTA"
    }</a>`;
  ...
  const leadForm = (title, btnText, accent) => `
    <form onsubmit="return false" style="margin-top:16px; display:grid; gap:10px">
      ...
      <button type="submit" style="padding:10px 14px;border:none;border-radius:10px;background:${accent};color:#fff;font-weight:700;cursor:pointer">${
    btnText || "Submit"
  }</button>
    </form>
  `;
  ...
  function render() {
    ...
    const v = {
      ...
      leadTitle: s.leadTitle || DEMO.leadTitle,
      leadBtn: s.leadBtn || DEMO.leadBtn,
    };
    ...
    `;
    const img = v.imgUrl
      ? `<img src="${v.imgUrl}" alt="" style="max-width:100%;display:block;margin:0 auto 14px;border-radius:12px"/>`
      : "";
    ...
    if (v.tpl === "t2") {
      html = `
        <div style="${shell}">
          ...
          ${btn(v.ctaText, v.ctaUrl, v.accent)}
          ${leadForm(v.leadTitle, v.leadBtn, v.accent)}
        </div>`;
    } else if (v.tpl === "t3") {
      html = `
        <div style="${shell}">
          <div style="display:grid;grid-template-columns:1.2fr 1fr;gap:18px">
            <div>
              <h1 style="margin:0 0 8px">${v.title}</h1>
              <h3 style="margin:0 0 12px;opacity:.85">${v.subtitle}</h3>
              <p style="margin:0 0 16px">${v.body}</p>
              ${btn(v.ctaText, v.ctaUrl, v.accent)}
              ${leadForm(v.leadTitle, v.leadBtn, v.accent)}
            </div>
